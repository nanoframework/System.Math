...
else
{
    ...
    git add "source\nanoFramework.System.Math\Properties\AssemblyInfo.cs"
...
if ($env:APPVEYOR_REPO_BRANCH -match "^dev*" -or $env:APPVEYOR_REPO_TAG -eq "true")
{
    ...
    # new branch name
    $newBranch = "$env:APPVEYOR_REPO_BRANCH-nfbot/update-version/nanoFramework.System.Math/$env:GitVersion_NuGetVersionV2"
    ...
    $assemblyFiles = (Get-ChildItem -Path ".\*" -Include "nf_native_system_math.cpp" -Recurse)
    ...
    {
        $commitMessage = "Update nanoFramework.System.Math version to $env:GitVersion_NuGetVersionV2"
